...
namespace cwing 
{
	...
	Button::Button(int x, int y, int w, int h, std::string txt):Component(x,y,w,h)
	{
		...
		upIcon = IMG_LoadTexture(sys.get_ren(), (constants::gResPath + "D:/random file/srandom images/upp.png").c_str() );
		downIcon = IMG_LoadTexture(sys.get_ren(), (constants::gResPath + "D:/random file/srandom images/down.png").c_str() );
	}
